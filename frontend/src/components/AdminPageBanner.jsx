...
const AdminPageBanner = ({ title, subtitle, icon: Icon }) => {
    return (
        <div className="relative h-64 overflow-hidden rounded-[2.5rem] mb-8 shrink-0 shadow-xl">
            <img src={GYM_HERO} alt="Dashboard Background" className="w-full h-full object-cover object-center" />
            <div className="absolute inset-0 bg-gradient-to-r from-slate-900/95 via-indigo-900/70 to-transparent" />
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                    {Icon && (
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-indigo-400 text-xs font-bold uppercase tracking-widest">MuscleHub Admin</span>
                        </div>
                    )}
