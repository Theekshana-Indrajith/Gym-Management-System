...
<<<<<<< Updated upstream
        ...
        const weight = parseFloat(selectedAImember.weight);
        const height = parseFloat(selectedAImember.height);

        if (!selectedAImember.gender || selectedAImember.gender.trim() === '') {
            alert("Nutritional Error: Gender specification is vital for metabolic rate calculation.");
            return;
        }

        if (isNaN(age) || age < 13 || age > 90) {
            alert("Validation Failed: Age must be between 13 and 90 for safe nutritional guidance.");
            return;
        }

        if (isNaN(weight) || weight < 30 || weight > 300) {
            alert("Validation Failed: Weight must be between 30kg and 300kg for caloric estimation.");
            return;
        }

        if (isNaN(height) || height < 100 || height > 250) {
            alert("Validation Failed: Height must be between 100cm and 250cm for BMI analysis.");
            return;
        }

        if (!selectedAImember.fitnessGoal || selectedAImember.fitnessGoal === '') {
...
<<<<<<< Updated upstream
                                        <input type="number" min="13" max="90" required className="w-full bg-white/5 p-3 rounded-xl text-white font-bold border border-white/5 focus:border-emerald-500 outline-none" value={selectedAImember.age || ''} onChange={e => setSelectedAIMember({ ...selectedAImember, age: e.target.value })} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Gender</label>
                                        <input required className="w-full bg-white/5 p-3 rounded-xl text-white font-bold border border-white/5 focus:border-emerald-500 outline-none" value={selectedAImember.gender || ''} onChange={e => setSelectedAIMember({ ...selectedAImember, gender: e.target.value })} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Weight (kg)</label>
                                        <input type="number" min="30" max="300" required className="w-full bg-white/5 p-3 rounded-xl text-white font-bold border border-white/5 focus:border-emerald-500 outline-none" value={selectedAImember.weight || ''} onChange={e => setSelectedAIMember({ ...selectedAImember, weight: e.target.value })} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Height (cm)</label>
                                        <input type="number" min="100" max="250" required className="w-full bg-white/5 p-3 rounded-xl text-white font-bold border border-white/5 focus:border-emerald-500 outline-none" value={selectedAImember.height || ''} onChange={e => setSelectedAIMember({ ...selectedAImember, height: e.target.value })} />
=======
...
<<<<<<< Updated upstream
                                        <select required className="w-full bg-white/5 p-3 rounded-xl text-white font-bold border border-white/5 outline-none focus:border-emerald-500" value={selectedAImember.fitnessGoal || ''} onChange={e => setSelectedAIMember({ ...selectedAImember, fitnessGoal: e.target.value })}>
                                            <option value="" className="bg-slate-900">Select Nutritional Goal</option>
=======
